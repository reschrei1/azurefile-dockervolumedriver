...
func mount(accountName, accountKey, shareName, mountpoint string) error {
	...
	cmd := exec.Command("mount", "-t", "cifs", fmt.Sprintf("//%s.file.core.cloudapi.de/%s", accountName, shareName), mountpoint, "-o", fmt.Sprintf("vers=3.0,username=%s,password=%s,dir_mode=0777,file_mode=0777", accountName, accountKey), "--verbose")
